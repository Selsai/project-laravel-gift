...
<head>
    ...
    <title>Ajouter un cadeau - {{ config('app.name', 'Laravel') }}</title>
...
<body class="bg-[#FDFDFC] dark:bg-[#0a0a0a] text-[#1b1b18] dark:text-[#EDEDEC] min-h-screen p-6">
    <div class="max-w-2xl mx-auto">
        <header class="mb-6">
            <h1 class="text-3xl font-bold mb-2">Ajouter un cadeau</h1>
        </header>

        <main class="bg-white dark:bg-[#161615] rounded-lg shadow p-6">
            <form action="{{ route('gifts.store') }}" method="POST" class="space-y-5">
                @csrf
